...
namespace Approval
{
    public partial class Warehouse : System.Web.UI.Page
    {
        ...
        private void LoadData()
        {
            ...
            if (drdept.SelectedIndex == 0 && drwarehouse.SelectedIndex == 0)
            {
                sql = "select a.*, b.fullname from it_note a left join IT_NOTE_USER b on a.user_kitting = b.ID " +
                    " where (a.status = 1 and a.checked=1 and kittingstatus <> 0) or (a.status = 1 and a.checked=1 and a.reasoncode = '5') order by note_date desc";

            }
            else if (drdept.SelectedIndex != 0 && drwarehouse.SelectedIndex != 0)
            {
                sql = "select a.*, b.fullname from it_note a left join IT_NOTE_USER b on a.user_kitting = b.ID " +
                    " where ((a.status = 1 and a.checked=1 and kittingstatus <> 0) or (a.status = 1 and a.checked=1 and a.reasoncode = '5')) and a.part='" + drdept.Text + "' and a.warehouse='" + drwarehouse.SelectedValue + "' order by note_date desc";
            }
            else
            {
                ...
                {
                    sql = "select a.*, b.fullname from it_note a left join IT_NOTE_USER b on a.user_kitting = b.ID " +
                        " where ((a.status = 1 and a.checked=1 and kittingstatus <> 0) or (a.status = 1 and a.checked=1 and a.reasoncode = '5'))  and a.part='" + drdept.Text + "' order by note_date desc";
                }
                if (drwarehouse.SelectedIndex != 0)
                {
                    sql = "select a.*, b.fullname from it_note a left join IT_NOTE_USER b on a.user_kitting = b.ID " +
                        " where ((a.status = 1 and a.checked=1 and kittingstatus <> 0) or (a.status = 1 and a.checked=1 and a.reasoncode = '5')) and a.warehouse='" + drwarehouse.SelectedValue + "' order by note_date desc";
                }
            }
        ...
        protected void bthisory_Click(object sender, EventArgs e)
        {
        ...
        protected void btnDetail_Click(object sender, EventArgs e)
        {
            foreach (GridViewRow row in grvNote.Rows)
            {
                ...
                if (chk.Checked)
                {
                    ...
                    Response.Redirect("WarehouseDetail.aspx?id=" + id);
                    break;
                }
            }
        }
