...
namespace Approval
{
    public partial class KittingList_M : System.Web.UI.Page
    {
        string per,wh;
            ...
            if (wh.Substring(0,1) =="5")
            {
                ...
                if (drdept.SelectedIndex == 0 && drwarehouse.SelectedIndex == 0)
                {
                    sql = "select a.*, b.fullname from it_note a left join IT_NOTE_USER b on a.user_kitting = b.ID " +
                        " where a.status = 1 and a.checked=1 and a.partcardstatus = 0 and a.kittingstatus = '0' and a.reasoncode <> 5 and a.warehouse = '5' order by note_date desc";

                }
                else if (drdept.SelectedIndex != 0 && drwarehouse.SelectedIndex != 0)
                {
                    sql = "select a.*, b.fullname from it_note a left join IT_NOTE_USER b on a.user_kitting = b.ID " +
                        " where a.status = 1 and a.checked=1 and a.partcardstatus = 0 and a.kittingstatus = '0' and a.reasoncode <> 5 and a.part='" + drdept.Text + "' and a.warehouse='" + drwarehouse.SelectedValue + "' and a.warehouse = '5' order by note_date desc";
                }
                else
                {
                    ...
                    {
                        sql = "select a.*, b.fullname from it_note a left join IT_NOTE_USER b on a.user_kitting = b.ID " +
                            " where a.status = 1 and a.checked=1 and a.partcardstatus = 0 and a.kittingstatus = '0' and a.reasoncode <> 5 and a.part='" + drdept.Text + "' and a.warehouse = '5' order by note_date desc";
                    }
                    if (drwarehouse.SelectedIndex != 0)
                    {
                        sql = "select a.*, b.fullname from it_note a left join IT_NOTE_USER b on a.user_kitting = b.ID " +
                            " where a.status = 1 and a.checked=1 and a.partcardstatus = 0 and a.kittingstatus = '0' and a.reasoncode <> 5 and a.warehouse='" + drwarehouse.SelectedValue + "' and a.warehouse = '5' order by note_date desc";
                    }
                }
            }
            else
            {
                //load vote khong phai PSNV
                if (drdept.SelectedIndex == 0 && drwarehouse.SelectedIndex == 0)
                {
                    sql = "select a.*, b.fullname from it_note a left join IT_NOTE_USER b on a.user_kitting = b.ID " +
                        " where a.status = 1 and a.checked=1 and a.partcardstatus = 0 and a.kittingstatus = '0' and a.reasoncode <> 5 and a.warehouse <> '5' order by note_date desc";
        ...
        protected void btnDetail_Click(object sender, EventArgs e)
        {
            foreach (GridViewRow row in grvNote.Rows)
            {
                ...
                if (chk.Checked)
                {
                    ...
                    Response.Redirect("KittingDetail_M.aspx?id=" + id);
                    break;
                }
            }
        }
