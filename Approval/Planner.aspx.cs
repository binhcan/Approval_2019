...
namespace Approval
{
    public partial class Planner : System.Web.UI.Page
    {
        ...
        public void LoadData()
        {
            ...
            {
                sql = "select * from plnaner where planner = '" + txtsearch.Text.Trim() + "' ";
            }

            DataTable tbl = data.GetDataTable(sql);
            grvPlanner.DataSource = tbl;
            grvPlanner.DataBind();
        }
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (HiddenField1.Value != "")
                {
                    ...
                    string sql = "update planner set planner='" + txtCode.Text.Trim() + "',description='" + txtName.Text.Trim() + "',update_date = getdate(), update_by = '" + use + "' where id_planner=" + idlo;
                ...
                {
                    string sql2 = "insert into Planner(Planner,description,Create_by,Create_date) "
        ...
        {
            foreach (GridViewRow row in grvPlanner.Rows)
            {
                CheckBox chk = (row.FindControl("cbSelectAll") as CheckBox);
                if (chk.Checked)
                {
                    int id = int.Parse(grvPlanner.DataKeys[row.RowIndex].Value.ToString());
                    HiddenField1.Value = id.ToString();
                    DataTable tam = data.GetDataTable("select * from planner where id_planner = " + id);
                    if (tam.Rows.Count > 0)
                    {
                        txtCode.Text = tam.Rows[0]["Planner"].ToString();
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            foreach (GridViewRow row in grvPlanner.Rows)
            {
                ...
                {
                    int id = int.Parse(grvPlanner.DataKeys[row.RowIndex].Value.ToString());
                    data.ExcuteQuery("delete from Planner where id_plnaner=" + id);
                    break;
                }
        ...
        {
            grvPlanner.PageIndex = e.NewPageIndex;
            int trang_thu = e.NewPageIndex;
            int so_dong = grvPlanner.PageSize;
        ...
        {
            string sql1 = "select * from Planner";
            ...
            Response.AddHeader("content-disposition", "attachment;filename= Planner_List.xls");
            Response.Charset = "";
